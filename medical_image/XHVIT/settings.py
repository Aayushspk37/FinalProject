...
# settings.py - Add this at the bottom
...
if GROQ_API_KEY:
    ...
    print(f"   Length: {len(GROQ_API_KEY)}")
    print(f"   Starts with: {GROQ_API_KEY[:10]}...")
    print(f"   Ends with: ...{GROQ_API_KEY[-5:]}")
else:
    ...
    if env_path.exists():
        ...
        with open(env_path, 'r') as f:
            content = f.read()
            # Show first 50 chars, hiding the full key
            if 'GROQ_API_KEY' in content:
                lines = content.split('\n')
                for line in lines:
                    if line.startswith('GROQ_API_KEY'):
                        if len(line) > 20:
                            print(f"     {line[:20]}...{line[-5:]}")
                        else:
                            print(f"     {line}")
                    else:
                        print(f"     {line}")
...
EMAIL_PORT = 587
EMAIL_USE_TLS = True
EMAIL_HOST_USER = 'aayushspk20@gmail.com'  # Replace with your email
EMAIL_HOST_PASSWORD = 'wktm ntvp ltzp yjos'  # Use App Password, not your regular password
DEFAULT_FROM_EMAIL = 'aayushspk20@gmail.com'
