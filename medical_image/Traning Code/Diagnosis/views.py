...
def generate_interpretation(request):
    try:
        ...
        print(f"📤 Sending request to Groq API for case: {case_id}")
        ...
        response = requests.post(
            ...
            json={
                ...
                'messages': [
                    {
                        'role': 'system',
                        'content': """You are a clinical AI assistant generating structured diagnostic reports.

RULES:
1. Use exactly 4 sections with these headings:
   - 1. PRIMARY FINDING
   - 2. DIFFERENTIAL DIAGNOSIS
   - 3. CLINICAL INTERPRETATION
   - 4. RECOMMENDED NEXT STEPS

2. Each section must have 3-4 bullet points
3. Each bullet point starts with a dash
4. Each bullet point is 1-2 sentences
5. Be concise and clinically sound
6. Use numbering for sections (1, 2, 3, 4)
7. No symbols except bullet dashes"""
                    },
                    {
                        'role': 'user',
                        'content': f"""Generate a structured clinical diagnostic report.
...
RULES:
- No asterisks, slashes, or symbols except bullet dashes
- Speak clinically, not casually
- Reference the confidence level and margin where relevant"""
                    }
                ],
                'temperature': 0.3,
                'max_tokens': 1200,
        ...
        if response.status_code == 200:
            ...
            if interpretation:
                print(f"✅ Success! Interpretation length: {len(interpretation)}")
...
def generate_local_interpretation_html(
    ...
    if confidence >= 90: conf_level = "High"
    elif confidence >= 75: conf_level = "Moderate"
    elif confidence >= 60: conf_level = "Fair"
    else: conf_level = "Low"

    if differential:
        diff_html = "".join([
            f"<li><strong>{d[0]}:</strong> {d[1]}%</li>"
            for d in differential
        ])
    else:
        diff_html = "<li>Differential data not available</li>"
        ...
        <div class="llm-body">
            ...
            <div class="llm-numbered-section">
                ...
                <div class="llm-section-content"><ul>
                    <li><strong>{diagnosis}</strong> detected with {confidence}% confidence</li>
                    <li>{info.get('description', 'N/A')}</li>
                    <li>Severity level: <strong>{info.get('severity', 'Unknown')}</strong></li>
                </ul></div>
            </div>

            <div class="llm-numbered-section">
                <div class="llm-section-number">2. DIFFERENTIAL DIAGNOSIS</div>
                <div class="llm-section-content"><ul>{diff_html}</ul></div>
            </div>

            <div class="llm-numbered-section">
                <div class="llm-section-number">3. CLINICAL INTERPRETATION</div>
                <div class="llm-section-content"><ul>
                    <li><strong>Common symptoms:</strong> {info.get('common_symptoms', 'N/A')}</li>
                    <li>Clinical correlation with patient history is required</li>
                    <li>Consider differential diagnoses in light of the decision margin</li>
                </ul></div>
            </div>

            <div class="llm-numbered-section">
                <div class="llm-section-number">4. RECOMMENDED NEXT STEPS</div>
                <div class="llm-section-content"><ul>
                    <li><strong>Standard treatment:</strong> {info.get('standard_treatment', 'Clinical correlation recommended')}</li>
                    <li>Specialist consultation based on severity</li>
                    <li>Additional diagnostic testing may be indicated</li>
                    <li>All findings require validation by a qualified clinician</li>
                </ul></div>
            </div>
        </div>
